...
Copy also applies rotation values configured in `expectedGateProperties` or `irat.gate` to exact, verified dynamic-password gates in the saved destination. Conflicting values or unsupported gate types stop the operation. No rotation value is inferred when neither field supplies one. Dry runs report the planned values without changing them. After changes, copy waits for the unsaved indicator to clear and reopens the saved destination with normal Open to verify persisted gate values.
