...
Prefer `--draft` over retyping the document: it writes a reviewable AE plan JSON with those titles, the case context, stems and options with their emphasis, detected answer keys, and gates. Hand transcription is where titles, emphasis, and figure order drift from the Source-of-Truth. Resolve every `_review` note and `TODO_` key, then preflight it.
