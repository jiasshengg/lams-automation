...
**Every question's correct answer must come from an explicit marker in the Source-of-Truth** — a uniformly bold/highlighted whole option, an explicit `Answer:` / `Correct answer:` / `Explanation -` line, or an equivalent explicit key naming the correct option. Never infer, guess, or supply a correct answer from outside/clinical knowledge, even when it looks obvious from the stem. Since iRAT has no `--draft` step to surface this automatically the way AE's `TODO_answerKey` does, check every question by hand before writing `answers`: if none of its options carry an explicit marker, do not author a weight for it. Stop and flag that exact question (its title/position and full text) to the user, and wait for their answer before including it in the request.
...
**Do not trust automatic caption detection alone — check every image by hand.** `isCaption` (`src/docx/media.ts`) only recognises a caption line that has Word's "Caption" paragraph style, a conventional prefix ("Figure/Table/Chart/Diagram/Image/Illustration …"), or is a hyperlink/bare URL; a plain attribution line such as "Medical gallery of Blausen Medical 2014" matches none of those and is invisible to it, so the image will be imported with an empty caption even though the source clearly prints one directly underneath. For every image, read the paragraph immediately following it in the source (or the extraction manifest) yourself, and when there is a line there that is not the next question/option/answer key, supply it as that image's `caption` explicitly. An explicit `caption`/`placement`/`altText`/`widthPx` on an image that is *also* found automatically (same file, e.g. because it is also at that question's own source position) now safely overrides the automatic value instead of being silently discarded — `deduplicate` in `src/docx/question-images.ts` used to key purely on image bytes and keep whichever copy happened to be built first, which dropped a manually-added caption with no warning (fixed 2026-09-25; regression test: `an explicit caption on an automatically-detected image is not lost to deduplication` in `tests/irat-question-images.spec.ts`). This applies identically to AE image captions.

When one Source-of-Truth image or case heading is printed once but introduces a **shared case narrative** for more than one adjacent question (a patient vignette, a `Qxx-yy relate to this case` heading, or the same phrasing without a heading), reproduce the image once per question that needs it rather than once for the group: give every such question its own `images` entry pointing at the same local file (`before`/`after` set per that question's own wording, and the same `caption` on every copy that shows the figure), and prepend the exact case heading/narrative text from the SoT (bold, `<br>` before the narrative) only to the first question of that group — later questions in the same case do not repeat the shared narrative or heading. Keep the SoT's own case-heading numbers verbatim even when the run's own question numbering differs (they identify the case in the source, not this lesson's authored positions).

**A different, narrower pattern: two adjacent questions pointing at the one image between them with no shared narrative at all** — e.g. "In the image below, which letter indicates a sulcus?" (Q19) followed directly by "In the brain above, which letter indicates the diencephalon?" (Q20), with nothing but the image between them, no case narrative, no heading. Because LAMS renders every question of an iRAT/tRAT on one continuous scrollable page, that single image is already visible to both questions without duplication — Q19's own "after" placement already puts it exactly where Q20's "above" wording expects it. Duplicating it here (as an earlier run did, corrected 2026-09-25) puts two identical copies of the same image back to back for no reason. For this pattern: add the image **once**, to the earlier question only (`placement` matching its own wording — typically `after`), and give the later question no `images` entry at all. This is the opposite of the shared-case-narrative rule above, and the two are easy to conflate — when two adjacent questions reference one image via complementary position words ("below"/"above" or equivalent) and there is no case narrative or heading connecting them, flag this specific case to the user and confirm single-vs-duplicate placement rather than silently picking one, since it is a judgment call the source text alone does not settle unambiguously. A shared case narrative (patient vignette, "relate to this case" heading) is the signal that duplication is correct instead — the presence of an actual narrative, not just complementary wording, is what distinguishes the two patterns.

**This is automatic when `irat.sourceDocx` is set and the SoT's questions are sequentially numbered** (`1.`, `2.`, `Q16.`, …, matching `NUMBERED_STEM` in `src/docx/question-numbering.ts`) — `resolveIratQuestionImages` duplicates a figure onto the following question automatically when nothing but a short caption/attribution line separates the image from that question's numbered stem (`sharedWithQuestionNumber` on the extracted image), and `applySotFormattingFromDocx` prepends a matching heading (`detectCaseHeadings`) onto whichever numbered question it introduces, once, safely re-runnable. Both require the two adjacent questions to carry real sequential numbers in the source; this codebase deliberately never guesses question boundaries in unnumbered prose (the same reason AE's own `assertSequentialQuestions` refuses non-sequential input rather than inferring it), so unnumbered SoT sections — like this project's `[iRAT-Test-Clinton]` run, where the shared-image/case-heading questions had no source numbering at all — still need the image/heading added by hand, exactly as described above, and still benefit from `sourceDocx` for inline-formatting reapplication once written.

**`resolveIratQuestionImages`'s automatic duplication does not know about the single-image "below"/"above" pattern above — it duplicates whenever `sharedWithQuestionNumber` fires, whether or not there is a case narrative.** So even for numbered SoT sections, check every automatically-duplicated image against the two patterns above: a genuine shared case narrative means the automatic duplication is correct and needs no change; a bare "below"/"above" pointer pair with no narrative means it duplicated a plain image-labelling pair that should only carry the image once, and the later question's automatic copy needs to be suppressed by hand (an empty explicit `images: []` on that question, since automatic detection cannot be turned off per-question otherwise) — and either way, flag which one it is to the user per the rule above.

**`detectCaseHeadings` only ever matches one exact wording — `CASE_RELATES_HEADING` in `src/docx/media.ts`, literally "Qxx[-yy] relate(s) to this case"** (any number/range, singular or plural) — by the same never-guess design as everything else in this codebase: it is a fixed regex, not a phrasing detector, so it is blind to every other way a source document can introduce a shared case. Real SoTs vary this wording freely — "refer to this description and image below for questions 28 and 29", "the following applies to Q4 and Q5", a heading that names the case but not the question numbers, or no heading at all and just a narrative paragraph followed by two questions that never reintroduce the patient. **Treat this as a reading-comprehension check, not a string match, every single run, on numbered and unnumbered sections alike**: whenever a narrative, image, or table appears once but a question's own wording only makes sense by reference to something already said or shown ("the above patient", "this rhythm", "in the image below/above", a bare pronoun with no antecedent in that question's own text, or a question that asks something new about a patient/scenario the previous question already fully introduced), that content is shared and must be duplicated per question exactly as described above — regardless of whether any heading is present, and regardless of what a present heading says. Do not wait for or expect the literal "relate to this case" string; find the boundary from what the questions actually depend on to make sense standalone, and duplicate the image/caption (and, only for the first question of the group, any narrative/heading) onto every question that needs it. When it is genuinely unclear whether two adjacent questions share one case or start independent ones, ask the user rather than guessing either way.
