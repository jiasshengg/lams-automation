...
import { LamsIratEditor, formattedHtml } from '../src/lams/irat-editor.js';
...
const request: IratRequest = {
  ...
  questions: [],
  advanced: { shuffleQuestions: false, shuffleAnswers: true, questionsNumbering: true, displayAllQuestions: true, answerJustification: false, confidenceLevels: false }
};
...
async function fixture(page: Page, existing = false, versionSave = true) {
  await page.route('https://irat.test/**', async route => {
    ...
    await route.fulfill({ contentType: 'text/html', body: `
      ...
      <script>
        ...
        function initEditor(id) { CKEDITOR.instances[id]={setData(value){this.data=value},fire(){}}; }
...
for (const count of [2, 5, 6]) {
  test(`creates a new MCQ with ${count} answers, scoring, formatting and feedback`, async ({ page }) => {
    ...
    const question = {
      ...
      feedback: 'Supplied rationale', prefixAnswersWithLetters: true, mandatory: false, fontFamily: 'Arial', fontSize: 12,
...
test('existing questions still require Save as new version', async ({ page }) => {
  ...
  await editor.updateQuestion({ title: 'Question 1', type: 'multiple-choice', marks: 1, content: 'Updated', mandatory: true, fontFamily: 'Arial', fontSize: 12, answers: [{ text: 'Yes', correct: true, weight: 100 }, { text: 'No', correct: false, weight: 0 }] });
...
test('never falls back to shared-question Save if version Save is missing', async ({ page }) => {
  ...
  await expect(editor.updateQuestion({ title: 'Question 1', type: 'multiple-choice', marks: 1, content: 'Updated', mandatory: true, fontFamily: 'Arial', fontSize: 12, answers: [{ text: 'Yes', correct: true, weight: 100 }, { text: 'No', correct: false, weight: 0 }] })).rejects.toThrow('refusing to save the shared question');
...
test('scientific formatting preserves sub/sup but never active markup or attributes', () => {
  expect(formattedHtml('10<sup>9</sup> H<sub>2</sub>O <img src=x onerror=alert(1)>', 'Arial', 12)).toContain('10<sup>9</sup> H<sub>2</sub>O');
  expect(formattedHtml('<b onclick="alert(1)">text</b>', 'Arial', 12)).not.toContain('onclick');
});
